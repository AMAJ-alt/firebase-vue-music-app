<template>
    <!-- Registration Form -->
    <div class="text-white text-center font-bold p-4 mb-4" v-if="reg_show_alert" :class="reg_alert_varient">
        {{ reg_Alert_msg }}
    </div>
    <vee-form :validation-schema="schema" @submit="register" :initial-values="userData">
        ...
        <div class="mb-3">
            ...
            <vee-field name="age" :bails="false" v-slot="{ field, errors }">
                <input type="number" v-bind="field" class="block w-full py-1.5 px-3 text-gray-800 border border-gray-300 transition
  ...
  data() {
    return {
      schema: {
        ...
        age: 'required|numeric|min_value:18|max_value:70',
        password: 'required|min:3|max:100',
        confirm_password: 'required|confirmed:@password',
        country: 'required|excluded:Japan',
        tos: 'required',
      },
      userData: {
        country: 'USA',
  ...
  methods: {
    async register(values) {
      ...
      try {
        ...
        this.reg_Alert_msg = 'An Unexpected error heppend ! try again .';
        return;
      }
