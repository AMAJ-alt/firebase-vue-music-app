<template>
    <!-- Login Form -->
    <div class="text-white text-center font-bold p-4 mb-4" :class="login_alert_varient" v-if="login_show_alert">
        {{ login_Alert_msg }}
    </div>
    <vee-form :validation-schema="loginSchema" @submit="login">
        ...
        <button type="submit" :disabled="login_in_submission" class="block w-full bg-purple-600 text-white py-1.5 px-3 rounded transition
                hover:bg-purple-700">
            Submit
        </button>
    </vee-form>
  ...
  methods: {
    async login(values) {
      ...
      try {
        ...
        this.login_Alert_msg = 'something happend!';
        console.log(error);
        return;
      }

      this.login_alert_varient = 'bg-green-500';
      this.login_Alert_msg = 'success ! you logged in .';
      console.log(values);
